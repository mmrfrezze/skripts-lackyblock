...
local LoadingScreen = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "Будущее уже здесь",
    ConfigurationSaving = {
        Enabled = true,
...
spawn(function()
    local messages = {
        ...
        "Готово к работе!",
        "Xiki Skript - будущее уже здесь!"
    }
    
    for i, msg in ipairs(messages) do
        LoadingScreen:SetLoadingTip(msg)
        wait(1.5)
    end
end)
...
CharTab:CreateSlider({
    ...
    Increment = 1,
    CurrentValue = 16,
    Callback = function(v) plr.Character.Humanoid.WalkSpeed = v end
})

CharTab:CreateButton({
    Name = "Reset Walk Speed",
    Callback = function() plr.Character.Humanoid.WalkSpeed = 16 end
})
...
CharTab:CreateSlider({
    ...
    Increment = 1,
    CurrentValue = 50,
    Callback = function(v) plr.Character.Humanoid.JumpPower = v end
})

CharTab:CreateButton({
    Name = "Reset Jump Power",
    Callback = function() plr.Character.Humanoid.JumpPower = 50 end
})
...
CharTab:CreateButton({
    Name = "Reset Gravity",
    Callback = function() workspace.Gravity = 196 end
})
...
CharTab:CreateButton({
    Name = "Reset FOV",
    Callback = function() cam.FieldOfView = 70 end
})
...
AdvTab:CreateToggle({
    ...
    CurrentValue = false,
    Callback = function(v) whitelistEn = v end
})
