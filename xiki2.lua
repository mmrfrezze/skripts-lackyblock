...
CharTab:CreateToggle({Name="Infinite Jumps", CurrentValue=false, Tooltip="Позволяет прыгать бесконечно", Callback=function(v) 
    infJump=v 
    if v then 
...
AdvTab:CreateToggle({Name="Kill Aura (NPCs)", CurrentValue=false, Callback=function(v)
    if v then
        equipBest()
